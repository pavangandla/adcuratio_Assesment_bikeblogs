...
def insert_data(request):
    ...
    if request.method =='POST' and request.FILES:
        BO = BikeForm(request.POST,request.FILES)
        if BO.is_valid():
            BO.save()
...
class BikeData(viewsets.ViewSet):
    def list(self,request):
        ADO=Bike.objects.all()
        SJD=BikeMS(ADO,many=True)
        d={'data':SJD.data}
        return render(request,'list.html',d)

    def retrieve(self,request,pk):
        TO=Bike.objects.get(pk=pk)
        SDO=BikeMS(TO)
        return Response(SDO.data)

    def update(self,request,pk):
        SPO=Bike.objects.get(pk=pk)
        SPD=BikeMS(SPO,data=request.data)
        if SPD.is_valid():
            SPD.save()
    ...
    def partial_update(self,request,pk):
        SPO=Bike.objects.get(pk=pk)
        SPD=BikeMS(SPO,data=request.data,partial=True)
        if SPD.is_valid():
            SPD.save()
            return Response({'Updated':'Product is updated'})
        else:
